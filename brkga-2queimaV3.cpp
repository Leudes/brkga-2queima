...
struct AlgorithmParameters {
  ...
  std::string output_file = "results3.csv";
  ...
  unsigned X_INTVL = 73;
  unsigned X_NUMBER = 2;
  unsigned MAX_GENS = 527;
  unsigned MAX_STAGT = 479;
  unsigned trials = 1;
};
...
int main(int argc, char *argv[]) {
    ...
    do {
      ...
      if ((++generation) % parameters.X_INTVL == 0) {
        algorithm.exchangeElite(parameters.X_NUMBER);
             ...
             stagnant_count < parameters.MAX_STAGT);
  ...
  std::cout << "\nTodos os resultados foram salvos em: "
            << parameters.output_file << std::endl;
#endif
...
void ensure_csv_header(const std::string &filename) {
  ...
  if (!file_exists) {
    file.open(filename);
    file << "graph_name,graph_order,graph_size,density,fitness_value,elapsed_"
            "time(microsecond)\n";
    file.close();
  }
}
...
void write_result_to_csv(const std::string &filename, const Result &result) {
  ...
  file << result.graph_name << "," << result.node_count << ","
       << result.edge_count << "," << result.graph_density << ","
       << result.fitness << "," << result.elapsed_time << "\n";
